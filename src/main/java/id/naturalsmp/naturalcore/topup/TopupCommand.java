...
public class TopupCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (target != null && target.isOnline()) {
            ...
            Bukkit.broadcastMessage(ChatUtils.colorize("  &#FFFF00❂ &#FFFF00&lTOPUP BERHASIL &#FFFF00❂"));
            Bukkit.broadcastMessage(ChatUtils.colorize("  &f" + target.getName() + " &7baru saja mendukung server!"));
            Bukkit.broadcastMessage(ChatUtils.colorize("  &7Terima kasih atas dukungannya. &#FFFF00❤"));
            Bukkit.broadcastMessage("");
        } else {
            sender.sendMessage(ChatUtils.colorize("&e[TopUp] Player " + playerName + " is offline. GUI will not be shown, but coins were given."));
        }
